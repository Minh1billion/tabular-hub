...
export function CreateWorkspaceCard({ isOpen, onOpen, onCancel, onSubmit, isSubmitting }: CreateWorkspaceCardProps) {
  ...
  if (isOpen) {
    return (
      <form
        ...
        className="bg-white border border-line rounded-card p-[18px] flex flex-col gap-3 min-h-[150px] justify-center"
      >
  ...
  return (
    <button
      type="button"
      onClick={onOpen}
      className="bg-cream-soft border border-dashed border-[#c7ccc4] rounded-card p-[18px] flex flex-col items-start justify-center gap-2.5 min-h-[150px] text-left transition-colors hover:border-brand"
    >
